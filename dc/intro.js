/* Static overview and appendix content for the Washington DC region.
   Spot data lives in dc/data.js (generated by tools/assemble.py from dc/parts/*.js). */
(function () {
  'use strict';

  var VERIFIED = '2026-09-05';

  /* The top info bar keeps only the season and the verification date. The first entry
     used to carry the actual dates of one specific trip, which pinned a long-lived
     handbook to a single journey; the sub-region and spot counts are already in the
     all-region map's heading, so don't repeat them. */
  var META = [
    ['季节参考', '秋色与闭馆节奏：十月中下旬'],
    ['核实日期', VERIFIED]
  ];

  /* ---------- Sunrise/sunset: variation across this region is tiny, one table is enough ---------- */
  var SUN = [
  ...
  /* ---------- Closed-day cross table: the easiest trap to fall into in this region ---------- */
  var CLOSED = [
  ...
  /* ---------- Tickets that must be grabbed in advance: sorted by days of lead time ---------- */
  var BOOK = [
  ...
  /* ---------- Overview ---------- */
  var OVERVIEW = [
    /* The order of days here is decided by the weekday, not by distance: several
       sub-regions shut down wholesale on a fixed day of the week. This card used to be
       written around the specific dates of one trip; it is now back to weekdays
       themselves — usable no matter which week the reader picks. */
    ...
    /* The next three are facts that vary by month rather than by any one trip, so they
       get their own block, each spelling out its season or month condition. */
