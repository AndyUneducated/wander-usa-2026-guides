/* Static overview and appendix content for the Yellowstone + Grand Teton region.
   Spot data lives in yellowstone/data.js (generated by tools/assemble.py from
   yellowstone/parts/*.js).

   This guide differs structurally from the other three: for the city guides the key
   constraint is which day a venue is closed, here it is the date things shut for the
   season — park roads and lodging close section by section from early October, and
   each operator closes on a different day. So appendix A is a closing calendar
   rather than a table of opening hours. */
(function () {
  'use strict';

  var VERIFIED = '2026-09-15';

  /* The top info bar keeps only the season and the verification date: concrete dates
     belong to one specific trip, and the spot count is already in the all-region
     map's heading. */
  var META = [
  ...
  /* ---------- Autumn 2026 closing calendar ----------
     Dates come from the official and concessioner pages already verified item by item
     in the sub-region cards. The Old Faithful Inn row keeps both the NPS and the
     Xanterra version even though they contradict each other, because this is exactly
     the kind of thing the reader has to confirm again before setting out. */
  var CLOSE = [
  ...
  /* ---------- Hard rules for wildlife and thermal areas ----------
     These two are the most common causes of injury and death in the park; the rules
     are enforced regulations, not suggestions. */
  var SAFETY = [
  ...
  /* ---------- Sunrise/sunset (example dates) ----------
     Old Faithful alone is enough: the park's east-west span produces less than ten
     minutes of difference, and the Jackson side is a few minutes earlier than here. */
  var SUN = [
  ...
  /* ---------- Departure-day recheck list ----------
     Sorted by how likely each is to change. The first two can change almost daily
     in autumn. */
  var RECHECK = [
    ['园内道路实时状态（含临时雪封）', '高',
