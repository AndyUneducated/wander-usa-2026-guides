/* Static overview and appendix content for the New York / New England region.
   Spot data lives in nyc/data.js (generated by tools/assemble.py from nyc/parts/*.js),
   7 sub-regions and 79 spots: Salem and North Shore, Boston and Cambridge,
   Hudson Valley and Catskills, Manhattan Uptown, Manhattan Midtown,
   Manhattan Downtown, Brooklyn and Queens.

   This file only holds region-level execution constraints — booking windows, closed
   days, seasonal boundaries, intercity transport. Per-spot hours and ticket prices
   always come from the access field in the parts files; here we only aggregate
   and sort across spots. */
(function () {
  'use strict';

  /* Date on which every live fact on this page (hours, prices, booking rules, event
     dates) was verified online. Spot-level data in the parts files was verified
     2026-09-13 to 2026-09-14, consistent with this. */
  var VERIFIED = '2026-09-15';

  /* The top info bar keeps only two entries: the seasonal framing and the verification
     date. Dates belong to the reader's own trip, and the sub-region and spot counts
     already appear elsewhere on the page, so don't repeat them here. */
  var META = [
    ['季节参考', '秋色与旺季：九月下旬至十月下旬'],
    ['核实日期', VERIFIED]
  ];

  /* ---------- Overview: the region's real execution constraints, ordered by importance ---------- */
  var OVERVIEW = [

    /* Booking goes first. The difference from DC: DC tickets are mostly free but
       timed, whereas here they are expensive, scarce, and released on rules that
       differ per venue, with three or four needing months of lead time. This block
       is first because it is the only constraint you cannot fix after departure. */
    ...
    /* Second block: closed days. Museum density here is extremely high, and unlike DC
       the closures do not cluster by city but by weekday — Tuesday and Wednesday each
       take out a completely different set of venues, so this needs a cross table. */
    ...
    /* Third block: Salem in October. The most year-sensitive block here — in 2026 the
       city cancelled the shuttles, while the hauntedhappenings.org transport page still
       describes the old shuttle plan. That is two official sources contradicting each
       other, so per this project's established practice we print both versions. */
    ...
    /* Fourth block: fall foliage. The region spans coastal lowland and two highland
       stretches whose peaks are two to three weeks apart, which is the single factor
       deciding where to go within one season. Written as a season range plus example
       dates, and it must say these forecasts come from a latitude/elevation model
       rather than field observation, or readers will treat them as a promise. */
    ...
    /* Fifth block: transport. The three bases sit about 190 miles apart, and choosing
       the intercity mode plus judging whether a city pass pays off are the two places
       money is most easily wasted here, so they are covered together. */
    ...
    /* Sixth block: a roundup of self-contradicting official sources. This kept coming
       up during the per-spot verification pass. Per this project's established practice
       we print both versions and do not pick one for the reader; they are collected
       here because scattered across individual cards they look like typos, and only
       together do they read as a systematic trait of this region. */
  ...
  /* Table rendering: identical implementation to dc/intro.js, first column bold.
     The outer tbl-scroll is emitted by this function, so the page script will not
  ...
  /* Sunrise/sunset: computed here with the NOAA solar position algorithm (atmospheric
     refraction taken as a 90.833° zenith angle), for three example autumn dates. The
     three places are at similar latitudes, so the differences come mostly from
     longitude: Salem and Boston are nearly in sync, New York City is about 12-13
     minutes later. */
  var SUN = [
  ...
  /* Closed-day cross table: organized by weekday rather than by city, because this
     region goes down in weekday-shaped blocks. Per-venue times come from access.hours
     in the corresponding parts file; this is only an aggregation. */
  var CLOSED = [
  ...
  /* Booking windows: sorted by how far ahead tickets are released, longest first —
     that is both the sort key and the order to act in. The two items that need no
     booking go last, because they are the ones guides most often wrongly list as
  ...
  /* Master table of hours and closed days: only the anchor spots of each sub-region
     plus the ones where timing itself is the constraint. In the last-entry column,
     anything the official source does not publish separately is recorded as such
  ...
  /* Departure-day recheck list, graded by how likely each item is to have changed.
     The four graded high all change year to year or month to month and are already
     known to disagree with what circulates online. */
  var RECHECK = [
