...
- 每个景点必须同时给两个评分：`must`（必去价值，主）与 `score`（摄影价值，辅）。
  页面卡头显示的是 `must`，摄影价值只作副行。
...
```js
{
  ...
  must: 5,                        // 【主评分】必去价值 0–5，允许 .5。见下方评分标准
...
| 分 | 含义 |
| --- | --- |
| **5** | 世界级、来了不去等于没来。错过会被人反问「你怎么没去」。如 Statue of Liberty、Old Faithful、The Met、National Air and Space Museum |
| **4** | 主流必去清单上的地标，时间够就该去。如 Grand Central、Boston Public Library、Arlington National Cemetery |
| **3** | 值得去，但属于「有兴趣再去」。通常是主题性强或需要特定兴趣的场馆与街区 |
| **2** | 顺路可看，不值得专程。多为街景、次要纪念物、纯拍照点位 |
| **1** | 只有特定人群会去（影视取景地、小众专题） |
| **0.5–1** | 已无法抵达或不存在的条目（同时要填 `gone`） |
